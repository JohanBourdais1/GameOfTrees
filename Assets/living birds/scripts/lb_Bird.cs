...
public class lb_Bird : MonoBehaviour {
	...
	IEnumerator FlyToTarget(GameObject target){
		if(Random.value < .5){
		...
		Vector3 vectorDirectionToTarget = (target.transform.position-transform.position).normalized;
		...
		if(vectorDirectionToTarget.y>.5f){
			...
			while(vectorDirectionToTarget.y>.5f){
				...
				vectorDirectionToTarget = (target.transform.position -transform.position).normalized;//reset the variable to reflect the actual target and not the temptarget
		...
		while(transform.rotation != finalRotation || distanceToTarget >= 1.5f){
			if(!paused){
				distanceToTarget = Vector3.Distance (transform.position,target.transform.position);
				vectorDirectionToTarget = (target.transform.position -transform.position).normalized;
		...
		Collider[] hitColliders = Physics.OverlapSphere(target.transform.position,0.05f*controller.birdScale);
		...
		while (distanceToTarget>0.05f*controller.birdScale){
			if(!paused){
				...
				transform.position = Vector3.SmoothDamp(transform.position,target.transform.position,ref vel,0.5f);
				t += Time.deltaTime;
				distanceToTarget = Vector3.Distance (transform.position,target.transform.position);
				if(t>2.0f){
		...
		if (target.name == "Right Hand Perch")
			PlayHapticLeftHand();
		if (target.name == "Left Hand Perch")
			PlayHapticRightHand();
    }

    IEnumerator FlyToTargetPosition(Vector3 target)
    {
        if (Random.value < .5)
        {
            GetComponent<AudioSource>().PlayOneShot(flyAway1, .1f);
        ...
        if (InputDevices.GetDeviceAtXRNode(XRNode.RightHand) != null)
        {
            ...
            if (InputDevices.GetDeviceAtXRNode(XRNode.RightHand).TryGetHapticCapabilities(out HapticCapabilities rightCapabilities) && rightCapabilities.supportsImpulse)
            {
                if (amplitude > 0f && amplitude < 1f)
                    InputDevices.GetDeviceAtXRNode(XRNode.RightHand).SendHapticImpulse(0u, amplitude, 0.3f);
            }
        }
    }

	void PlayHapticRightHand()
	{
        if (InputDevices.GetDeviceAtXRNode(XRNode.LeftHand) != null)
        {
            ...
            {
                if (amplitude > 0f && amplitude < 1f)
                    InputDevices.GetDeviceAtXRNode(XRNode.LeftHand).SendHapticImpulse(0u, amplitude, 0.3f);
            }
        }
    }
	...
	void OnGroundBehaviors(){
		...
		if(idle){
			...
			if (Random.value < Time.deltaTime*.06){
				FlyAway ();
			}
		}
	}
	...
	void Flee(){
		if(!dead){
			StopCoroutine("FlyToTarget");
			StopCoroutine("FlyToTargetPosition");
			GetComponent<AudioSource>().Stop();
			anim.Play(flyAnimationHash);
			Vector3 farAwayTarget = transform.position;
			farAwayTarget += new Vector3(Random.Range (-100,100)*controller.birdScale,30*controller.birdScale,Random.Range (-100,100)*controller.birdScale);
			//farAwayTarget += Vector3.up * 20;
			StartCoroutine("FlyToTargetPosition",farAwayTarget);
		}
	}
